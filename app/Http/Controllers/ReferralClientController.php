...
class ReferralClientController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = ReferralClient::where('form_type','=','referrals')->where('referral_clients.created_by','=',Auth::id())->get();
    ...
    public function sresponse(Request $request)
    {
        ...
        // print_r($data); exit;
        if ($request->ajax()) {
            $data = ReferralClient::where('form_type','!=','referrals')->join('users', 'referral_clients.created_by', '=', 'users.id')->where('referral_clients.created_by','=',Auth::id())->orWhere('users.reporting_user_id','=',Auth::id())->get();
            return DataTables::of($data)
