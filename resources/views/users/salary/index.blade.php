@extends('layouts.app')

@section('content')
<div class="container-xxl container-p-y">

    <h4 class="fw-bold mb-3">
        Salary – {{ $user->name }} (FY {{ $fy }}) 
    </h4>
    <p>Joining Date: {{ $user->joining_date }} | Confirmation Date: {{ $user->confirm_date }} </p>

    <form method="POST" action="{{ route('users.salary.store', $user->id) }}">
        @csrf
        <input type="hidden" name="financial_year" value="{{ $fy }}">

        @php $netSalary = $user->net_salary ?? 0; @endphp

        <div class="card">
            <div class="table-responsive">
                <table class="table table-striped align-middle mb-0">
                    <thead>
                        <tr>
                            <th width="150">Month</th>
                            <th width="150">Gross</th>
                            <th width="150">LOP</th>
                            <th width="180">TDS</th>
                            <th width="180">Net Credited</th>
                            <th width="150">PT</th>
                            <th width="150">PF</th>
                            <th width="150">Total Cost</th>
                        </tr>
                    </thead>

                    <tbody>
                        @foreach($months as $m)

                        @php
                            $monthDate = \Carbon\Carbon::create($m['year'], $m['month'], 1);

                            $joiningDate = $user->joining_date
                                ? \Carbon\Carbon::parse($user->joining_date)->startOfMonth()
                                : null;

                            // Hide months before joining
                            if ($joiningDate && $monthDate->lt($joiningDate)) {
                                continue;
                            }
                            $confirmationDate = $user->confirm_date
                                ? \Carbon\Carbon::parse($user->confirm_date)
                                : null;

                            $pf = ($confirmationDate &&
                                   $monthDate->gte($confirmationDate) &&
                                   $user->employment_status === 'confirmed')
                                ? ($user->pf_employee ?? 0) + ($user->pf_employer ?? 0)
                                : 0;

                            // PT Logic
                        if ($user->gender === 'female' && $user->current_ctc < 25000) {

                            $pt = 0;

                        } else {

                            // February PT = 300
                            if ($monthDate->month == 2) {

                                $pt = 300;

                            } else {

                                $pt = $user->professional_tax ?? 0;
                            }
                        }

                            $credited = $m['salary_credited'] ?? 0;

                            $tds = $m['tds'] ?? 0;

                            $lop = $m['extra_deduction'] ?? 0;

                            $totalCost = $credited + $tds + $pt + $pf;

                            $gross = $totalCost + $lop;
                        @endphp

                        <tr>
                            <td>{{ $m['label'] }}</td>

                            <td>
                                
                                    <input type="number"
                                        step="0.01"
                                        class="form-control gross-input"
                                        name="salary[{{ $m['year'] }}][{{ $m['month'] }}][gross]"
                                        value="{{ $gross }}">
                                <!-- @if($m['month'] == 4 && $m['year'] == 2025)

                                @else

                                    ₹ {{ number_format($gross,2) }}

                                    <input type="hidden"
                                        name="salary[{{ $m['year'] }}][{{ $m['month'] }}][gross]"
                                        value="{{ $gross }}">

                                @endif -->
                            </td>

                            <td>
                                ₹ <span class="lop-display">
                                        {{ number_format($lop,2) }}
                                    </span>

                                    <input type="hidden"
                                        class="lop-input"
                                        name="salary[{{ $m['year'] }}][{{ $m['month'] }}][extra_deduction]"
                                        value="{{ $lop }}">
                                <!-- @if($m['month'] == 4 && $m['year'] == 2025)
                                @else

                                    ₹ <span class="lop-display">
                                        {{ number_format($lop,2) }}
                                    </span>

                                    <input type="hidden"
                                        name="salary[{{ $m['year'] }}][{{ $m['month'] }}][extra_deduction]"
                                        value="{{ $lop }}">

                                @endif -->

                            </td>
                            <td>
                                <input type="number"
                                    step="0.01"
                                    min="0"
                                    class="form-control tds-input"
                                    name="salary[{{ $m['year'] }}][{{ $m['month'] }}][tds]"
                                    value="{{ $tds }}">
                            </td>

                            <td>
                                <input type="number"
                                    step="0.01"
                                    min="0"
                                    class="form-control salary-input"
                                    data-standard="{{ $netSalary }}"
                                    data-pt="{{ $pt }}"
                                    data-pf="{{ $pf }}"
                                    name="salary[{{ $m['year'] }}][{{ $m['month'] }}][salary_credited]"
                                    value="{{ $credited }}"
                                    {{ !$m['enabled'] ? 'disabled' : '' }}>
                            </td>

                            <td>₹ {{ number_format($pt,2) }}</td>
                            <td>₹ {{ number_format($pf,2) }}</td>

                            <td>
                                ₹ <span class="row-total">
                                    {{ number_format($totalCost,2) }}
                                </span>
                            </td>
                        </tr>

                        @endforeach
                    </tbody>

                    <tfoot>
                        <tr class="fw-bold bg-light">
                            <th>Total</th>
                            <th id="gross-total">0.00</th>
                            <th id="lop-total">0.00</th>
                            <th id="tds-total">0.00</th>
                            <th id="net-total">0.00</th>
                            <th id="pt-total">0.00</th>
                            <th id="pf-total">0.00</th>
                            <th id="cost-total">0.00</th>
                        </tr>
                    </tfoot>
                </table>
            </div>
        </div>

        <div class="mt-3 text-end">
            <button type="submit" class="btn btn-primary">
                Save Salary
            </button>
        </div>
    </form>
</div>
@endsection
@section('script')
<script>

function calculateSalary() {

    let grossTotal = 0,
        lopTotal = 0,
        tdsTotal = 0,
        netTotal = 0,
        ptTotal = 0,
        pfTotal = 0,
        costTotal = 0;

    document.querySelectorAll('tbody tr').forEach(row => {

        let salaryInput = row.querySelector('.salary-input');
        let tdsInput = row.querySelector('.tds-input');
        let grossInput = row.querySelector('.gross-input');
        let lopInput = row.querySelector('.lop-input');

        if (!salaryInput || salaryInput.disabled) return;

        let pt = parseFloat(salaryInput.dataset.pt) || 0;
        let pf = parseFloat(salaryInput.dataset.pf) || 0;

        let credited = parseFloat(salaryInput.value) || 0;
        let tds = parseFloat(tdsInput.value) || 0;
        let gross = parseFloat(grossInput.value) || 0;

        /*
        |--------------------------------------------------------------------------
        | TOTAL COST
        |--------------------------------------------------------------------------
        */

        let totalCost = credited + tds + pt + pf;

        /*
        |--------------------------------------------------------------------------
        | LOP CALCULATION
        |--------------------------------------------------------------------------
        */

        let lop = gross - totalCost;

        if (lop < 0) {
            lop = 0;
        }

        /*
        |--------------------------------------------------------------------------
        | UPDATE HIDDEN LOP FIELD
        |--------------------------------------------------------------------------
        */

        if (lopInput) {
            lopInput.value = lop.toFixed(2);
        }

        /*
        |--------------------------------------------------------------------------
        | UPDATE LOP DISPLAY
        |--------------------------------------------------------------------------
        */

        let lopDisplay = row.querySelector('.lop-display');

        if (lopDisplay) {

            lopDisplay.innerText =
                lop.toLocaleString('en-IN', {
                    minimumFractionDigits: 2
                });
        }

        /*
        |--------------------------------------------------------------------------
        | UPDATE TOTAL COST DISPLAY
        |--------------------------------------------------------------------------
        */

        row.querySelector('.row-total').innerText =
            totalCost.toLocaleString('en-IN', {
                minimumFractionDigits: 2
            });

        /*
        |--------------------------------------------------------------------------
        | FOOTER TOTALS
        |--------------------------------------------------------------------------
        */

        grossTotal += gross;
        lopTotal += lop;
        tdsTotal += tds;
        netTotal += credited;
        ptTotal += pt;
        pfTotal += pf;
        costTotal += totalCost;
    });

    /*
    |--------------------------------------------------------------------------
    | FOOTER UPDATE
    |--------------------------------------------------------------------------
    */

    document.getElementById('gross-total').innerText =
        grossTotal.toLocaleString('en-IN', {
            minimumFractionDigits: 2
        });

    document.getElementById('lop-total').innerText =
        lopTotal.toLocaleString('en-IN', {
            minimumFractionDigits: 2
        });

    document.getElementById('tds-total').innerText =
        tdsTotal.toLocaleString('en-IN', {
            minimumFractionDigits: 2
        });

    document.getElementById('net-total').innerText =
        netTotal.toLocaleString('en-IN', {
            minimumFractionDigits: 2
        });

    document.getElementById('pt-total').innerText =
        ptTotal.toLocaleString('en-IN', {
            minimumFractionDigits: 2
        });

    document.getElementById('pf-total').innerText =
        pfTotal.toLocaleString('en-IN', {
            minimumFractionDigits: 2
        });

    document.getElementById('cost-total').innerText =
        costTotal.toLocaleString('en-IN', {
            minimumFractionDigits: 2
        });
}

/*
|--------------------------------------------------------------------------
| EVENTS
|--------------------------------------------------------------------------
*/

document.addEventListener('input', e => {

    if (
        e.target.classList.contains('salary-input') ||
        e.target.classList.contains('tds-input') ||
        e.target.classList.contains('gross-input')
    ) {
        calculateSalary();
    }

});

/*
|--------------------------------------------------------------------------
| INITIAL LOAD
|--------------------------------------------------------------------------
*/

document.addEventListener('DOMContentLoaded', calculateSalary);

</script>
@endsection
